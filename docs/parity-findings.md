...
|---|---|---|---|
| P-24 | — | The composition canvas is placed on a different plane than upstream would choose, on a CRTC with more than one primary. Upstream scores a primary `+8` for the composition layer and then, after allocation, explicitly moves it to the first **unassigned primary** on the CRTC — its header says the composition layer "is always assigned to the primary". `compose_unassigned` here adds the canvas after allocation on the first unassigned **non-cursor** plane in kernel enumeration order, whatever its type. | **Reasoned, not reproduced.** With one primary the two agree: free, and both take it; taken by a layer, and neither has a primary to move to. The divergence needs a *second* primary that is not the first free plane — upstream skips to it, this takes whichever free plane the kernel lists first, typically an overlay. Different plane, different trace, and on a driver that pins its primary's zpos, a different stacking slot. Not reachable on vkms, which has one primary per CRTC, so `composition-overflow` diffs clean and says nothing about it. The hardware is real and the corpus counts it: **57 CRTCs across six drivers** — rockchip 28, omapdrm 8, vc4 8, msm_drm 6, msm 5, rcar-du 2 — including two of the drivers the lab is being built around. Surveyed by `crtcs_with_more_than_one_primary`. Left open deliberately: which placement is *right* is a question about the hardware, not about the code, and answering it by picking one now would be guessing. It wants a board with the shape. Asked upstream as [drm-cxx#257](https://github.com/jwinarske/drm-cxx/issues/257): whether "always on a primary" is a contract rooted in how these drivers treat primary planes, or a preference expressed twice. Their answer decides which way this resolves. |
...
| P-25 | The cursor buffer was sized from `PlaneCapabilities::cursor_max_w`, which nothing fills — `PlaneRegistry::probe` zeroes it on every plane. So the cap was always absent, `preferred_size` took its 64px fallback on every atomic path, and the doc comment promising "a plane that wants 256 gets 256" was describing something that could not happen. | **Fixed.** Measured on vkms, which advertises `512x512`: the renderer settled on `64x64`, four times coarser than the hardware offers, with a blurry pointer as the only symptom. `cursor_cap` now asks the device for `DRM_CAP_CURSOR_WIDTH`/`HEIGHT` — which is what the reference does at the same point, so this was a port defect rather than a divergence to reproduce — and takes the smaller of the two, since the buffer is square. The settled size went `64` to `256`, the largest ladder rung under the advertised cap. Pinned by `a_renderer_settles_on_a_plane_and_a_size_vkms`, which now asserts the rung against the driver's own cap instead of printing the size; verified by making the cap unfindable, which fails it with the number in the message. The fix also exposed a latent bug in `a_cursor_is_drawn_and_staged_vkms`: it expected a negative `CRTC_X` as 64-bit two's complement, and a 64px cursor kept the coordinate positive so the encoding never showed. `CRTC_X` is signed **32-bit**. Upstream's field is dead the same way, filed as [drm-cxx#258](https://github.com/jwinarske/drm-cxx/issues/258) — its sizing is correct because `create()` asks the device rather than the registry. |
| P-26 | The scene reserves a plane for the composition canvas on one trigger — overflow, when there are more layers than candidate planes. The reference has **two**. The missing one is the *primary anchor*: a primary whose zpos is pinned, with no live layer eligible to land on it. Nothing then assigns the primary, so the disable pass writes `FB_ID = 0` and `CRTC_ID = 0` to it in every `TEST_ONLY`, and a driver enforcing "an active CRTC has an armed primary" refuses each one. Every test fails, the allocator places nothing, and the whole scene falls through to software composition — still correct, still arriving, silently not using the hardware. | **Fixed.** Found by reading the reference's source rather than asking it: `layer_scene.cpp:1327` documents the trigger and the failure, and `docs/hardware.md` records the amdgpu behaviour behind it ("PRIMARY must stay armed during atomic `TEST_ONLY`"). amdgpu pins its primary at zpos 2 and ARM Mali display cores pin similarly, so the layout the reference's own examples use — `zpos >= 3` for every layer — makes nothing eligible and reaches this directly. `canvas_reservation` now checks the anchor before overflow, since the anchor is about a commit being refused where overflow is only about a layer being composited unnecessarily. A layer is eligible when its zpos matches the pin or when it has none, the second because `plane_score`'s primary bonus will put it there. **Unreachable on vkms**, which exposes no zpos property at all, so no lane could have caught it and none can regress it — the four unit cases stand in, and the anchor one fails when the trigger is disabled. Not reproduced on hardware: amdgpu here is held by a desktop session. |
