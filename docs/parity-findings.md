...
| P-18 | — | The composition canvas is armed on a plane no `TEST_ONLY` ever included. The allocator validates the layer assignment; `compose_unassigned` then picks a free plane for the canvas and `build_plan` adds it. On hardware that lights fewer planes than it advertises the assembled frame is refused, and a refused frame puts *nothing* on screen. | **Partly fixed.** Reproduced on a Radxa ZERO 3 (rockchip VOP2, RK3566): 3 planes eligible on the CRTC, **2 simultaneously usable** — measured with raw ioctls at 64x64 and 640x480, with and without `zpos` writes, always 2. The failing frame armed three objects: two carrying layers, one carrying the canvas. `canvas_reservation` did not help because it triggers on `layers > candidates`, a count proxy blind to a hardware limit *below* the candidate count. Now a second allocation pass runs when composition actually turns out to be needed, reserving a plane for the canvas — verified A/B on the board, 3 passed becomes 4. What remains: when the hardware limit is at or below the candidate count, `assigned` is already the hardware maximum, so `assigned + canvas` overshoots whatever is reserved. The real fix is to include the canvas plane in the assignments the allocator tests, which is a design change rather than a patch. Filed as [drm-cxx#244](https://github.com/jwinarske/drm-cxx/issues/244); upstream has the same structure. |
