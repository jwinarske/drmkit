...
| `tests/unit/test_buffer_mapping.cpp` | `drmkit-dumb` `src/tests.rs` | partial | Invariant 6. `DefaultIsEmptyAndDestructorIsNoOp` is ported twice over — as behaviour (`default_buffer_is_empty`, `empty_buffer_yields_an_empty_mapping`) and as a type property, since `mapping_has_no_destructor` asserts `needs_drop` is false and so fails the moment anyone adds a `Drop` impl or a field carrying one. The other four are C++ move semantics with no Rust equivalent: `DestructorCallsUnmapOnce` counts a destructor this type does not have, and `MoveConstructTransfersUnmapOwnership`, `MoveAssignDestroysExistingTarget` and `SelfMoveAssignmentIsHarmless` all test that a moved-from husk stays safe — a state Rust's move semantics do not produce. `move_preserves_an_empty_buffer` covers what remains of the intent. |
| `tests/unit/test_buffer_ring.cpp` | `drmkit-present` `buffer_ring.rs` | ported | All seven upstream cases, plus two the reference has no equivalent of: a ring asked for zero slots is clamped to one, and releasing the scanning slot is ignored rather than freeing what is on screen. Upstream is one binary with a `CHECK` counter; these are separate cases, so a break says which contract broke. `Repaint` is an enum here rather than `{bool full; vector<Rect> region;}` — that struct can hold a full repaint *and* a region, and a caller reading the region without checking the flag under-paints. |
...
| `tests/unit/test_negotiate.cpp` | `drmkit-present` `negotiate.rs` | ported | All five upstream cases, plus two the reference has no equivalent of: that a half-turn filters nothing — the half of `transposes()` that would silently over-filter — and that the three fixture modifiers really are one of each bandwidth class, without which the ranking case would pass on three modifiers that classify identically. Rotation is an explicit argument rather than defaulting to `Rotate0`: a caller who has not thought about rotation is the one whose 90-degree linear layer is refused at commit, by a message naming neither. The AMD DCC case is covered by `drmkit-fmt`'s own `rotation_compatible` tests. |
...
| `tests/unit/test_scene.cpp` | `drmkit-scene` `tests.rs` | partial | Handle identity, the dirty-flag contract, placement and report defaults, and the hint accessors. Outstanding, and each for a structural reason rather than an oversight: the seven `IfChanged` cases test per-field `set_*_if_changed` setters where this has one `set_display` that never dirties, since geometry does not change plane scoring; `CreateFailsGracefullyOnInvalidDevice` and `OnNonDrmFd` need a `Device` built from a bad fd, which is not representable; and the `GbmBufferSource` cases wait on `drmkit-scene-gbm` |
...
| `tests/integration/test_dumb_ring_source.cpp` | `drmkit-present` `dumb_ring.rs` + `tests/dumb_ring_device.rs` | ported | The eight-frame loop, asserting the same thing it does: fresh slots repaint fully and a reused one eventually asks for a partial, which is the assertion a ring that always answered `Full` would fail. Plus two the reference documents and does not test — that slots are allocated as the ring grows rather than up front, and that acquiring without painting blocks rather than re-presenting the last frame. The device is a `paint` parameter rather than a stored `const Device*`: storing a borrow would put a lifetime into every scene holding one, and it makes session resume a no-op instead of a pointer swap. Upstream returns zero when it finds no card, so its lane is green whether or not it tested anything; these bail through `drmkit_testkit::skipped`. |
...
| `tests/integration/test_layer_scene_composition_vkms.cpp` | `drmkit-scene` `tests/composition_vkms.rs` | partial |  Seven cases against the reference's three. `EmptyColdStartDoesNotPoisonWarmStart` is ported in shape but cannot pin what it pins there: the reference's `EAGAIN` came from an empty assignment left marked valid, and three separate things stop that here -- `previous_valid` cleared on an empty assignment, the warm path's `!previous.is_empty()`, and `has_new_layer` being true for every layer when the cache is empty. Removing all three still passes. What it does pin is that a fully-composited frame stays fully composited, which a composition that rescues nothing fails. `DmaBufSourceRescuedByGpuImport` waits on `drmkit-scene-gbm` |
