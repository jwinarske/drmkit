...
|---|---|---|---|
| P-10 | — | The scene never CPU-waits on an acquire fence when the assigned plane has no `IN_FENCE_FD` property. `AcquiredBuffer::acquire_fence` documents both routes — hand it to KMS where the plane supports it, otherwise wait before committing — and only the first is implemented, so on such a plane a fenced buffer is committed without anything having waited. | Open — needs the fence reachable from the emit path, which currently sees only the lowered property bag. Unreachable on vkms, whose planes all advertise `IN_FENCE_FD`. |
